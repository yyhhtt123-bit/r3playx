...
async function main() {
  ...
  if (argv.x64 || argv.arm64 || argv.arm) {
    ...
    if (isWindows) {
      ...
      // On a Linux server, we only build for the host architecture (x64).
      // The arm64 build was failing due to missing toolchain.
      await build('x64')
      // await build('arm64') // <-- MODIFIED: Commented out the failing arm64 build.
    }
  }
}
